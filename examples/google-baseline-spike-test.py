...
print("Google Gemini Baseline / Spike / Reroute")
print("=" * 70)
...
# CLIENT
# ==========================================================
SERVICE = "google-baseline-spike-test"
ENV = "dev"
...
for i in range(BASELINE_SIZE):
    ...
    run_stream(
        prompt="Write exactly these sixteen words and nothing else: one two three four five six seven eight nine ten eleven twelve thirteen fourteen fifteen sixteen.",
...
spike_prompt = "Write exactly these sixteen words and nothing else: one two three four five six seven eight nine ten eleven twelve thirteen fourteen fifteen sixteen."
...
if not control_allowed and is_tech_artifact(execution_path):
    ...
    provider_valid = execution_metadata.get("provider") == "google"
    initial_model_valid = execution_metadata.get("model") == PRIMARY_MODEL
    transport_valid = True
    target_consistency_valid = execution_target == {}
else:
    provider_valid = execution.get("initial_provider") == "google"
    initial_model_valid = execution.get("initial_model") == PRIMARY_MODEL

    if execution_changed:
        transport_valid = (
            execution_target.get("from_transport") == "google"
            and execution_target.get("to_transport") == "google"
            and execution_target.get("from_provider") == "google"
            and execution_target.get("to_provider") == "google"
        )
        ...
        transport_valid = (
            execution.get("initial_provider") == "google"
            and execution.get("final_provider") == "google"
        )
...
assert provider_valid, (
    ...
    f"expected=google, actual={execution.get('initial_provider')}"
)
