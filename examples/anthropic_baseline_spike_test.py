...
from wail.runtime.context import get_current_trace_id
...
def _observed_execute(self, *args, **kwargs):
    ...
    execution_log.append({
        ...
        "requested_model": (kwargs.get("original_kwargs") or {}).get("model"),
        "executed_model": getattr(target, "model", None),
        "execution_target": dict(result.get("execution_target", {}) or {}),
    })
    return result
...
print("Anthropic Baseline / Spike / Reroute")
print("=" * 70)
...
# CLIENT
# ==========================================================
SERVICE = "anthropic-baseline-spike-test"
ENV = "dev"
...
def run_stream(
    ...
    actual = len(execution_log) - before

    if actual != 1:
        raise AssertionError(
            "Expected exactly one ControlExecutor execution for one request. "
            f"actual={actual}, model={model}"
        )

    record = execution_log[-1]
    return record.get("executed_model") or model, record
...
for i in range(BASELINE_SIZE):
    ...
    run_stream(
        prompt="Write exactly these twelve words and nothing else: one two three four five six seven eight nine ten eleven twelve.",
...
spike_prompt = "Write exactly these twelve words and nothing else: one two three four five six seven eight nine ten eleven twelve."
...
# ==========================================================
# POST CONTROL - CLEAN N+1 + STABILITY OBSERVATIONS
# ==========================================================
...
def collect_service_traces():
    ...
    for path in latest_trace_files():
        if Path(path).stat().st_mtime < TEST_STARTED_AT:
            continue
...
for i in range(len(service_traces) - 1):
    ...
    required_pairs.append(pair_record)

    if not pair_valid:
        failed_pairs.append(pair_record)
        continue
...
if not required_pairs:
    ...
    raise AssertionError(
        "FAILED: No required CRITICAL INFRA_HEALTH control decision found. "
        f"plan={active_plan}, "
...
if failed_pairs:
    ...
    raise AssertionError(
        "FAILED: One or more required N -> N+1 control pairs failed. "
        f"plan={active_plan}, "
        f"reroute_entitled={reroute_entitled}, "
        f"control_allowed={control_allowed}, "
        f"required_pairs={len(required_pairs)}, "
...
if decision_trace is None:
    ...
    raise AssertionError(
        "FAILED: Required control decisions were found, "
        "but no valid execution pair was produced. "
        f"pairs={required_pairs}"
    )
...
if not control_allowed and is_tech_artifact(execution_path):
    ...
    provider_valid = execution_metadata.get("provider") == "anthropic"
    initial_model_valid = execution_metadata.get("model") == PRIMARY_MODEL
    transport_valid = True
    target_consistency_valid = execution_target == {}
else:
    provider_valid = execution.get("initial_provider") == "anthropic"
    ...
    if execution_target:
        transport_valid = (
            ...
            and execution_target.get("from_provider") == "anthropic"
            and execution_target.get("to_provider") == "anthropic"
        )
        ...
        transport_valid = (
            execution.get("initial_provider") == "anthropic"
            and execution.get("final_provider") == "anthropic"
        )
        target_consistency_valid = (
...
assert provider_valid, (
    ...
    f"expected=anthropic, actual={execution.get('initial_provider')}"
)
