...
wail.configure(
    service="multi-provider-openai-anthropic-test",
    env="dev",
)
...
# ==========================================================
# PHASE 2 - CONTROLLED INFRA DEGRADATION (OPENAI ONLY)
# ==========================================================

print("\n=== CONTROLLED INFRA DEGRADATION (OPENAI) ===")
...
if not reroute_pending:
    raise AssertionError(
        "FAILED: Controlled OpenAI degradation did not produce "
        "a pending REROUTE."
    )
...
def collect_service_traces():
    ...
    for path in current_run_files:
        ...
        if ctx.get("service") != "multi-provider-openai-anthropic-test":
            continue
...
if len(service_traces) < 2:
    raise AssertionError(
        "FAILED: Not enough OpenAI-origin service artifacts for "
    ...
    if decision_n == "reroute":
        ...
        if reroute_entitled:
            reroute_applied = (
                ...
                and target_next.get("from_transport") == "openai"
                and target_next.get("to_transport") == "anthropic"
